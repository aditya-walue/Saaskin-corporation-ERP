...
def link_deal_address_to_customer(deal, customer):
	...
	the deal's own Contact; falls back to the deal's own `address` field
	(a Link to an Address record -- saaskin_erp.enrichment builds one from
	scraped text) if no Contact-linked Address exists. Does nothing if
	neither is available -- never invents an address from nothing.
	"""
	if frappe.db.get_value("Customer", customer, "customer_primary_address"):
		return

	address_name = get_contact_linked_address(deal) or deal.get("address")
